...
class Fox(Character):
    ...
    def logic(self):
        if self.compare_state(AI.state.ActionState.Wait) or\
                self.compare_state(AI.state.ActionState.Landing):
            self.shorthop_nair(1)
        elif self.compare_state(AI.state.ActionState.RebirthWait):
